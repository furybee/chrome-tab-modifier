...
function processTitle(currentUrl, currentTitle, rule) {
  ...
  if (rule.tab.title_matcher) {
    try {
      const titleMatches = currentTitle.match(
        new RegExp(rule.tab.title_matcher, "g"),
      );
      if (titleMatches) {
  ...
  if (rule.tab.url_matcher) {
    try {
      const urlMatches = currentUrl.match(
        new RegExp(rule.tab.url_matcher, "g"),
      );
      if (urlMatches) {
...
// Core logic with clear structure
chrome.storage.local.get(STORAGE_KEY, (items) => {
  ...
  function applyRule() {
    ...
    if (rule.tab.title) {
      const newTitle = processTitle(location.href, document.title, rule);
