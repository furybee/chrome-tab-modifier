...
chrome.storage.local.get("tab_modifier", function (items) {
  ...
  let tab_modifier = items.tab_modifier,
    rule = null,
    processPage;
  ...
  processPage = function () {
    ...
    for (let i = 0; i < tab_modifier.rules.length; i++) {
      if (
        ...
        switch (tab_modifier.rules[i].detection) {
          case "STARTS":
            if (
              location.href.startsWith(tab_modifier.rules[i].url_fragment) ===
              true
            ) {
              rule = tab_modifier.rules[i];
              break;
          ...
          case "ENDS":
            if (
              location.href.endsWith(tab_modifier.rules[i].url_fragment) ===
              true
            ) {
              rule = tab_modifier.rules[i];
              break;
    ...
    let getTextBySelector, updateTitle, processTitle, processIcon;
    ...
    processIcon = function (new_icon) {
      ...
      icon =
        /^(https?|data):/.test(new_icon) === true
          ? new_icon
          : chrome.extension.getURL("/assets/" + new_icon);
