<template>
    ...
    <label class="form-control w-full max-w-xs flex-1">
      ...
      <div v-if="showHelp" class="label">
        <span class="text-xs label-text-alt"
          >Give an explicit name, just for you</span
        >
      </div>
    </label>
  ...
  <div v-if="isFirstPartFilled" class="mt-6">
    ...
    <label class="form-control w-full flex-1 mt-4">
      ...
      <div class="label">
        <span v-if="showHelp" class="text-xs label-text-alt">
          You can inject any DOM content with {selector}. Examples: {title} for
          website title, {h1}, {#id}, {.class}, etc. (Read the help section)
        </span>
      </div>
    </label>
    ...
    <div class="flex gap-2 mt-4">
      <label class="form-control w-full max-w-xs flex-0">
        ...
        <CustomSelect
          v-model="currentRule.tab.group_id"
    ...
    <details class="mt-6">
      ...
      <div class="flex gap-2">
        <label class="form-control w-full flex-1">
          ...
          <div class="label">
            <span v-if="showHelp" class="text-xs label-text-alt">
              Regular expression to search string fragments in title (read the
              help section)</span
            >
          </div>
        </label>
        ...
        <label class="form-control w-full flex-1">
          ...
          <div class="label">
            <span v-if="showHelp" class="text-xs label-text-alt">
              Regular expression to search string fragments in URL (read the
              help section)</span
            >
          </div>
        </label>
  ...
  <div class="modal-action items-center">
    ...
    <button class="btn btn-sm btn-outline btn-primary ml-4 group" @click="save">
      Save
      <span v-if="showHelp">
        <kbd class="kbd kbd-xs group-hover:text-neutral group-hover:bg-primary"
          >⌘</kbd
        ><kbd class="kbd kbd-xs group-hover:text-neutral group-hover:bg-primary"
          >S</kbd
        >
...
const isFirstPartFilled = computed(() => {
  return (
    ...
    currentRule.value.detection &&
    currentRule.value.url_fragment
  );
});
